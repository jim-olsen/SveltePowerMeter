...
def dataReport():
    try:
        ...
        weatherData.update({
            ...
            'rain_total': float(request.form.get('rrain_piezo', 0)) / 25.4,
...
#            'inTemp_F': request.form.get('tempinf', 0),
            ...
            'daily_rain': float(request.form.get('drain_piezo', 0)) / 25.4,
            'wind_average': request.form.get('windspeedmph', 0),
            'rain_in': float(request.form.get('drain_piezo', 0)) / 25.4,
            ...
            'hourRain_in': float(request.form.get('hrain_piezo', 0)) / 25.4,
            'rain24_in': float(request.form.get('drain_piezo', 0)) / 25.4,
            'dayRain_in': float(request.form.get('drain_piezo', 0)) / 25.4,
            'weekRain_in': float(request.form.get('wrain_piezo', 0)) / 25.4,
