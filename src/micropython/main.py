...
def main():
    ...
    while True:
        try:
            current_battery_voltage = (adc_battery_voltage.read() / ((3.273 / 3.3) * 65535)) * 36
            ...
            current_battery_load = (((adc_battery_load.read() / 65535) * 3.3) - 1.65) * CURRENT_SENSOR_FACTOR
            ...
            current_load = (((adc_load.read() / 65535) * 3.3) - 1.65) * CURRENT_SENSOR_FACTOR
            ...
            ble.advertise_stop()
            ble.advertise(data=advertisement.advertise_data("{:.2f}".format(current_battery_voltage) + ":" +
                                     "{:.2f}".format(current_battery_load) + ":" + "{:.2f}*".format(current_load)),
                      connectable=False)
