<script>
    ...
    export let value = 'on';
    export let cycleButton = false;
    export let cycleButtonAction = doNothing();
...
{#if design == 'inner'}
    <div class="s s--inner" style="display:flex; justify-content:space-between;flex-flow: row;">
...
{:else if design == 'slider'}
    <div class="s s--slider" style="display:flex; justify-content:space-between;flex-flow: row;font-size:{fontSize}px;">
        <span id={`switch-${uniqueID}`}>{label}</span>
        <div style="display:flex;flex-flow:row;align-items:center;gap:15px;">
            <button
                    role="switch"
                    class="slider-button"
                    aria-checked={checked}
...
{:else}
    <div class="s s--multi" style="display:flex; justify-content:space-between;flex-flow: row;">
        <div role='radiogroup'
...
<style>
    ...
    .s--inner .slider-button span {
        user-select: none;
        pointer-events: none;
        padding: 0.25em;
    }

    .s--inner .slider-button:focus {
    ...
    .s--slider .slider-button[aria-checked='true'] {
        background-color: var(--accent-color)
    }

    .s--slider .slider-button[aria-checked='true']::before {
        transform: translateX(1.3em);
        transition: transform 0.3s;
    }

    .s--slider .slider-button:focus {
