<script>
    ...
    import {batteryBankVoltageGraphData, powerGraphDuration} from "../../stores.svelte.js";
    ...
    let graphData = [];
    const unsubscribeGraph = batteryBankVoltageGraphData.subscribe(data => {
        graphData = [];
        let groupData = [];
        if (data.hasOwnProperty("time")) {
            data?.time?.forEach((d, i) => {
                let value = data?.voltage?.[i] ? data?.voltage?.[i] : 0;
