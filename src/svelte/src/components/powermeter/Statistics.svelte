...
<div style="display:flex; flex-flow:column">
    <table>
        <tbody>
            <tr>
                <td>Today Usage</td>
                <td>{statsData?.day_load_wh ? statsData?.day_load_wh?.toFixed(2) : '---'} <small>wh</small></td>
                <td>Avg Use</td>
                <td>{statsData?.avg_load ? statsData?.avg_load?.toFixed(2) : '---'} <small>wh</small></td>
            </tr>
            <tr>
                <td>Today Solar</td>
                <td>{statsData?.day_solar_wh ? statsData?.day_solar_wh?.toFixed(2) : '---'} <small>wh</small></td>
                <td>Avg Solar</td>
                <td>{statsData?.avg_solar ? statsData?.avg_solar?.toFixed(2) : '---'} <small>wh</small></td>
            </tr>
        <tr>
            <td>Today Net</td>
            <td>{((statsData?.day_solar_wh ? statsData?.day_solar_wh : 0) - (statsData?.day_load_wh ? statsData?.day_load_wh : 0))?.toFixed(2)} <small>wh</small></td>
            <td>Avg Net</td>
            <td>{statsData?.avg_net ? statsData?.avg_net?.toFixed(2) : '---'} <small>wh</small></td>
        </tr>
        <tr>
            <td>Yesterday Net</td>
            <td>{statsData?.thirty_days_net ? statsData?.thirty_days_net?.[29]?.toFixed(2) : '---'} <small>wh</small></td>
            <td>Yesterday Use</td>
            <td>{statsData?.thirty_days_load ? statsData?.thirty_days_load?.[29]?.toFixed(2) : '---'} <small>wh</small></td>
        </tr>
        <tr>
            <td>Today Batt Use</td>
            <td>{statsData?.day_batt_wh ? statsData?.day_batt_wh?.toFixed(2) : '---'} <small>wh</small></td>
            <td>Yest Batt Use</td>
            <td>{statsData?.thirty_days_batt_wh?.[29] ? statsData?.thirty_days_batt_wh?.[29]?.toFixed(2) : '---'} <small>wh</small></td>
        </tr>
        <tr>
            <td>Five Day Net</td>
