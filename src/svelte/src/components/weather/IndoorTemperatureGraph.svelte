<script>
    ...
    import {temperatureGraphData} from "../../stores";
    ...
    const unsubscribeGraph = temperatureGraphData.subscribe(data => {
        ...
        if (data.hasOwnProperty("time")) {
            data?.time?.forEach((d, i) => {
                let value = data?.inTemp_F?.[i] ? data?.inTemp_F?.[i] : 0;
                graphData.unshift({x: Date.parse(d.slice(0, -4)), y: value})
                value = data?.outTemp_F?.[i] ? data?.outTemp_F?.[i] : 0;
...
<div style="display:flex; flex-flow:row">
    <LineChart XAxisTitle="Time" YAxisTitle="Temp F" dataset={graphData} additionalDataSet={secondGraphData}
